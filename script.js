...
function loadEmployees() {
    ...
    if (storedEmployees) {
        employees = JSON.parse(storedEmployees).map(emp => {
            const employee = new Employee(emp.name, emp.weekdayRate);
            employee.unavailableDates = emp.unavailableDates.map(date => new Date(date));
            employee.recurringUnavailability = emp.recurringUnavailability;
            return employee;
        });
    }
}
...
// Function to prompt new users for employee names
function promptForEmployees() {
    let employeeNames = prompt("Please enter employee names, separated by commas:");
    if (employeeNames) {
        employeeNames.split(',').forEach(name => {
            const trimmedName = name.trim();
            if (trimmedName) {
                employees.push(new Employee(trimmedName, standardWeekdayRate));
            }
        });
        saveEmployees();
    }
...
// Wrap all the code that interacts with DOM elements in a DOMContentLoaded event listener
document.addEventListener('DOMContentLoaded', function() {
    ...
    const manageEmployeesBtn = document.getElementById('manageEmployees');
    ...
    const manageHolidaysBtn = document.getElementById('manageHolidays');
    const holidayModal = document.getElementById('holidayModal');
    const holidayEmployeeSelect = document.getElementById('holidayEmployeeSelect');
    ...
    if (generateRotaBtn) generateRotaBtn.addEventListener('click', generateRota);
    if (addUnavailableDateBtn) addUnavailableDateBtn.addEventListener('click', addUnavailableDate);
    unavailableTypeRadios.forEach(radio => {
        radio.addEventListener('change', toggleUnavailableInputs);
    });
    if (manageEmployeesBtn) manageEmployeesBtn.addEventListener('click', openEmployeeModal);
    if (addNewEmployeeBtn) addNewEmployeeBtn.addEventListener('click', addNewEmployee);
    if (manageHolidaysBtn) manageHolidaysBtn.addEventListener('click', openHolidayModal);
    ...
    // If no employees are loaded, prompt the user to enter employee names
    if (employees.length === 0) {
        promptForEmployees();
    }

    // Update the UI with the loaded or newly added employees
    updateEmployeeList();
    updateEmployeeSelect();
    updateEmployeeListInModal();
    ...
    function generateRota() {
        ...
        while (currentDate <= endDate) {
            ...
            if (isWeekend) {
                ...
                const duty = createShift(new Date(currentDate), 'Weekday Duty', '16:30', '08:30', 1);
    ...
    // Add these functions for managing employees
    function openEmployeeModal() {
        employeeModal.style.display = 'flex';
        updateEmployeeListInModal();
    }

    function updateEmployeeListInModal() {
        const employeeListDiv = document.getElementById('employeeList');
        employeeListDiv.innerHTML = '';
        employees.forEach((employee, index) => {
            ...
            employeeItem.innerHTML = `
                ...
                <button class="removeEmployeeBtn" data-index="${index}">Remove</button>
            `;
            employeeListDiv.appendChild(employeeItem);
        });

        // Add event listeners for remove buttons
        document.querySelectorAll('.removeEmployeeBtn').forEach(btn => {
            btn.addEventListener('click', removeEmployee);
        });
    }
    ...
    function addNewEmployee() {
        ...
        if (name) {
            ...
            showCustomAlert('Please enter an employee name.', newEmployeeNameInput);
        }
    }
    ...
    // Add these new functions for holiday management
    function openHolidayModal() {
        console.log('openHolidayModal function called');
        const holidayModal = document.getElementById('holidayModal');
        console.log('holidayModal:', holidayModal);
        
        if (holidayModal) {
            holidayModal.style.display = 'flex';
            console.log('Holiday modal display set to flex');
            updateHolidayEmployeeSelect();
            updateHolidayList();
        } else {
            console.error('Holiday modal not found');
            console.log('All elements with class "modal":', document.querySelectorAll('.modal'));
            console.log('All elements in the document:', document.body.innerHTML);
        }
    }

    function updateHolidayEmployeeSelect() {
        holidayEmployeeSelect.innerHTML = '<option value="">Select an employee</option>';
        employees.forEach((employee, index) => {
            const option = document.createElement('option');
            option.value = employee.name;
            option.textContent = employee.name;
            holidayEmployeeSelect.appendChild(option);
        });
    }

    function addHoliday() {
        const employeeName = holidayEmployeeSelect.value;
        const startDate = holidayStartDate.value;
        const endDate = holidayEndDate.value;

        if (!employeeName || !startDate || !endDate) {
            showCustomAlert('Please fill in all fields', addHolidayBtn);
            return;
        }

        try {
            addHolidayWithValidation(employeeName, startDate, endDate);
            updateHolidayList();
            holidayStartDate.value = '';
            holidayEndDate.value = '';
        } catch (error) {
            showCustomAlert(error.message, addHolidayBtn);
        }
    }
    ...
    function updateHolidayList() {
        ...
        getAllEmployeesWithHolidays().forEach(employeeName => {
            const employeeHolidays = getEmployeeHolidays(employeeName);
            if (employeeHolidays.length > 0) {
                const employeeDiv = document.createElement('div');
                employeeDiv.innerHTML = `<h3>${employeeName}</h3>`;
                employeeHolidays.forEach(holiday => {
                    const holidayDiv = document.createElement('div');
                    holidayDiv.className = 'holiday-item';
                    holidayDiv.innerHTML = `
                        ${formatDate(holiday.startDate)} - ${formatDate(holiday.endDate)}
                        <button class="removeHolidayBtn" data-employee="${employeeName}" data-start="${holiday.startDate.toISOString()}" data-end="${holiday.endDate.toISOString()}">Remove</button>
                    `;
                    employeeDiv.appendChild(holidayDiv);
                });
                holidayList.appendChild(employeeDiv);
            }
        });

        // Add event listeners for remove buttons
        document.querySelectorAll('.removeHolidayBtn').forEach(btn => {
            btn.addEventListener('click', removeHolidayEvent);
        });
    }

    function removeHolidayEvent(event) {
        ...
        removeHoliday(employeeName, startDate, endDate);
        updateHolidayList();
    }
